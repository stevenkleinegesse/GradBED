...
def initialise_design(
        modelparams, device, bounds=[0, 50],
        d_init=None, init_type='uniform', sort=False):
    ...
    if d_init is None:
        if init_type == 'uniform':
            d_init = np.random.uniform(
                bounds[0], bounds[-1], size=DIM).reshape(-1, 1)
        elif init_type == 'latin':
            ls = np.linspace(bounds[0], bounds[1], DIM + 1)
            d_init = np.array([np.random.uniform(
                ls[i], ls[i + 1]) for i in range(DIM)]).reshape(-1, 1)
    else:
        pass
    ...
    d = torch.tensor(
        d_init, dtype=torch.float, device=device, requires_grad=True)

    return d, d_init


def initialise_model(
        modelparams, device, task='pe',
        model_init_state=None, batch_norm=False):
