...
class FullyConnected_BatchNorm(nn.Module):
    ...
    def __init__(self, var1_dim, var2_dim, L=1, H=10):
        ...
        super(FullyConnected_BatchNorm, self).__init__()
    ...
    def forward(self, var1, var2):
        ...
        # Initial layer over random variables
        hidden = F.relu(self.bn0(self.fc_var1(var1) + self.fc_var2(var2)))

        # All subsequent layers
        for idx in range(len(self.layers) - 1):
            hidden = F.relu(self.bn_layers[idx](self.layers[idx](hidden)))
