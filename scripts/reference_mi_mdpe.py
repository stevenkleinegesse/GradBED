...
def likelihood_MDPE(d, data_full, priorsamples, kde, model=1):

    # get predictions depending on the model
    if model == 1:
        pred_all = priorsamples[:,0] + priorsamples[:,1] * d.reshape(-1, 1)
    elif model == 2:
        dabs = np.abs(d)
        delta = 1e-4
        dabs[dabs < delta] = delta
        pred_all = priorsamples[:,0] + priorsamples[:,1] * np.log(dabs.reshape(-1, 1))
    elif model == 3:
        pred_all = priorsamples[:,0] + priorsamples[:,1] * np.sqrt(np.abs(d.reshape(-1, 1)))
    else:
        raise NotImplemented()

    # compute differences to data y
    diff = data_full - pred_all
    # compute the density based on a kde over noise
    pdfs_inner = np.array([kde(delta) for delta in diff])
    # compute the joint likelihoods across all dimensions 
    pdfs_product = np.prod(pdfs_inner, axis=0)
...
# compute MDPE likelihoods
like_mdpe_1 = likelihood_MDPE(d, y_1, p1_samples, density, model=1)
like_mdpe_2 = likelihood_MDPE(d, y_2, p2_samples, density, model=2)
like_mdpe_3 = likelihood_MDPE(d, y_3, p3_samples, density, model=3)
...
idx_good = [i for i in range(len(m2)) if m2[i]!=0]

# compute log probabilities; entropy term 'l' and cross-entropy term 'm'
l_mdpe = (np.log(like_mdpe_1[idx_good]) + np.log(like_mdpe_2[idx_good]) + np.log(like_mdpe_3[idx_good])) / 3
m_mdpe = (np.log(m1[idx_good]) + np.log(m2[idx_good]) + np.log(m3[idx_good])) / 3

# log ratio
mi_int = l_mdpe - m_mdpe

# ----- SAVE ------ #

save_dict_sub = dict()
save_dict_sub['like_1_mdpe'] = like_mdpe_1
save_dict_sub['like_2_mdpe'] = like_mdpe_2
save_dict_sub['like_3_mdpe'] = like_mdpe_3
save_dict_sub['like_1'] = like_1
save_dict_sub['like_2'] = like_2
save_dict_sub['like_3'] = like_3
save_dict_sub['like_1in2'] = like_1in2
save_dict_sub['like_1in3'] = like_1in3
save_dict_sub['like_2in1'] = like_2in1
save_dict_sub['like_2in3'] = like_2in3
save_dict_sub['like_3in1'] = like_3in1
save_dict_sub['like_3in2'] = like_3in2
save_dict_sub['design'] = d
save_dict_sub['MC_NUM'] = MC_NUM
save_dict_sub['mi_estimate'] = np.mean(mi_int)
save_dict_sub['mi_error'] = np.std(mi_int) / np.sqrt(len(mi_int))
torch.save(save_dict_sub, '../data/reference_mdpe_test.pt')