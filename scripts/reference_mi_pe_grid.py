...
GS = 21
design_grid = np.linspace(-2, 2, GS).reshape(-1, 1, 1)
...
for d in tqdm(design_grid):
    ...
    # compute likelihoods
    like_pdfs = list()
    for idx in tqdm(range(d.shape[0]), disable=not trainbar):
        ys_d = ys[:,idx]
    ...
    marg_data = Parallel(n_jobs=int(N_JOBS))(
        delayed(get_margdata)(y, d, prior_shuffle) for y in tqdm(ys, disable=not trainbar))
    ...
    # save
    data_like_grid.append(like)
    data_marg_grid.append(marg)

# ----- SAVE ------ #

save_dict = dict()
save_dict['MC_NUM'] = MC_NUM
save_dict['design_grid'] = design_grid
save_dict['like_grid'] = data_like_grid
save_dict['marg_grid'] = data_marg_grid
torch.save(save_dict, '../data/reference_pe_grid_small.pt')