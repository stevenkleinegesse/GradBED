...
from scipy.integrate import simps as scs
...
def compute_refpost_on_grid(d, y_obs, theta_grid, tt, pdf_prior, density, bar=True):
    ...
    Z = scs(scs(prod.reshape(len(tt), len(tt)), tt, axis=1), tt)
    pdf_post = prod / Z
...
def compute_pred_post(y, d, y_T_grid, future_d, theta_grid, tt, pdf_prior, density, bar=True):
    ...
    for yt in tqdm(y_T_grid, disable=not bar):
        ...
        kde = density(diff)
        joint = kde * pdf_post
        pdf = scs(scs(joint.reshape(len(tt), len(tt)), tt, axis=1), tt)
        pred_post.append(pdf)
    pred_post = np.array(pred_post)
    
    # normalize posterior prediction
    Z_post = scs(pred_post, y_T_grid)
...
Y = simulator_linear(d_fp, X_truth)
...
save_dict['y_obs_data'] = Y

part = 4
torch.save(save_dict, '../data/reference_post_fp_{}.pt'.format(part))
